...
class IMGParser:
    def __init__(self):
        ...
        # self.lower_wlane = np.array([0,0,190])
        # self.upper_wlane = np.array([30,60,255])

        self.lower_wlane = np.array([10, 0, 200])
        self.upper_wlane = np.array([30, 50, 255])

        self.lower_ylane = np.array([20, 100, 100])
...
if __name__ == '__main__':
    ...
    curve_learner = CURVEFit(order=3, lane_width=3.6 ,y_margin=1.5, x_range=30, min_pts=20)
    ...
    while not rospy.is_shutdown():
        ...
        if image_parser.img_bgr is not None:
            ...
            cv2.imshow("binarization", img_lane)

            cv2.imshow("birdview", cv2.resize(img_lane_fit, (params_cam["WIDTH"]*2, params_cam["HEIGHT"]*2)))
